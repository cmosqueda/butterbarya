...
export default function Dashboard() {
  const { attendance, payslips, settings } = usePayroll();
  const periods = useMemo(() => Array.from(new Set([periodId(todayText()), ...attendance.map((row) => row.payrollPeriod), ...payslips.map((row) => row.payrollPeriod)])).sort().reverse(), [attendance, payslips]);
  ...
  return <PageShell eyebrow="Personal payroll" title="Here’s your pay, at a glance." action={<div className="period-picker"><label htmlFor="period">Pay period</label><select id="period" value={selected} onChange={(e) => setSelected(e.target.value)}>{periods.map((item) => <option key={item} value={item}>{periodLabel(item)}</option>)}</select></div>}>
    ...
    <section className="lower-grid">
      <article className="surface recent-card">
        ...
        {rows.length ? <div className="activity-list">{rows.slice(0, 4).map((row) => <div className="activity-row" key={row.id}><div className="date-tile"><strong>{new Date(`${row.date}T00:00:00`).getDate()}</strong><span>{new Intl.DateTimeFormat('en', { month: 'short' }).format(new Date(`${row.date}T00:00:00`))}</span></div><div className="activity-main"><strong>{row.status}</strong><span>{row.timeIn && row.timeOut ? `${row.timeIn} – ${row.timeOut}` : row.remarks || 'No hours required'}</span></div><div className="activity-hours"><strong>{row.workedHours.toFixed(1)}h</strong><span>{row.lateMinutes ? `${row.lateMinutes}m late` : 'On time'}</span></div></div>)}</div> : <EmptyState text="No time logged for this cutoff yet." link="/attendance" label="Log your first day" />}
      </article>
